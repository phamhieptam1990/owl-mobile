...
class PlannedForDateScreen extends StatefulWidget {
  PlannedForDateScreen({Key? key}) : super(key: key);
  @override
  _PlannedForDateScreenState createState() => _PlannedForDateScreenState();
}

class _PlannedForDateScreenState extends State<PlannedForDateScreen>
  ...
  Future<void> handleFetchData() async {
    try {
      final Response response = await this
          ._collecitonService
          .getPagingPlannedDateNew(_collectionProvider.getCurrentPage,
              date: selectedDate);
      ...
      if (response.data != null) {
        ...
        if (Utils.checkIsNotNull(dateTimeSystemInt)) {
          DateTime dateTimeSystem = Utils.converLongToDate(
              Utils.convertTimeStampToDateEnhance(dateTimeSystemInt ?? '') ?? 0);
  ...
  void _onTapShareFile(TicketModel data) async {
    ...
    try {
      ...
      // Fix 6: Safe json decoding with try-catch
      GenerateJobResponse? generateJobResponse;
      try {
        generateJobResponse = GenerateJobResponse.fromJson(jsonDecode(response?.body ??''));
      } catch (e) {
        print('Error decoding response: $e');
        WidgetCommon.dismissLoading();
        WidgetCommon.showSnackbarErrorGet('Dữ liệu phản hồi không hợp lệ');
        return;
      }
      if (generateJobResponse.status == 0 && generateJobResponse.data != null) {
  ...
  Future<void> downloadCheckedList() async {
    ...
    try {
      if (_collectionProvider.getLstTicket.isNotEmpty) {
  ...
  buildActionSheet(TicketModel item) async {
    if (item.customerData == null) {
      ...
      final aggId = item?.aggId;
      if (aggId == null) {
        return;
      }
      final Response resCustomer =
          await _customerService.getContactByAggId(aggId);
